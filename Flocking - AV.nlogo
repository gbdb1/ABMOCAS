...
to go
  ...
  if first question = "5" [
    ...
    ask herdanimals [updatehappiness]     ; this procedure results in updates in happiness for the herdanimals
  ]
;  if (first question = "0")
;    [ ask herdanimals [ flock ]
;      set cohesion-flag true
;      ask one-of herdanimals
;        [ if not shade-of? color yellow
;          [ set normal-flag false ] ]
;      if not normal-flag
;        [ ask herdanimals [ set color yellow - 2 + random 7 ] ] ]
...
;      if not normal-flag
;        [ ask herdanimals [ set color yellow - 2 + random 7 ] ]
;      ask herdanimals [ set color approximate-hsb heading 100 100 ]
;      set cohesion-flag true ]
;  ; "Are the boids in separation or aligned mode?"
;  if first question = "2"
;    [ ask herdanimals [ flock-mode ]
;      set cohesion-flag true ]
;  ; "Do all of the boids end up following the same leader?"
;  if first question = "3"
;    [ if (cohesion-flag)
;      [ ask herdanimals [ set color one-of base-colors ]
;        set cohesion-flag false ]
;      ask herdanimals
;        [ flock
;          if any? flockmates [ set color [ color ] of nearest-neighbor ] ] ]
;  ifelse first question = "4"
;    [ ask herdanimals [ flock-shape-mode ] ;; herdanimals cohering have line shape, separating have default shape
;      ask herdanimals [ set color approximate-hsb heading 100 100 ]  ;; color herdanimals according to their heading
;      set cohesion-flag true ]
;  [ ask herdanimals [ set shape "default" ] ]
  tick
end
...
to updatehappiness
  ifelse distance nearest-neighbor < (2 * ( d1 - d0 )) [
    set happiness ( 1 / ( 1 + exp ( -  k0 * ( (distance nearest-neighbor)- x0 ))))
  ][
    set happiness ( 1 - ( 1 / ( 1 + exp ( -  k1 * ( (distance nearest-neighbor) - x1 )))))
  ]
  set unhappiness (1 - happiness)
end

to flock  ;; herdanimal procedure
  find-flockmates
  if any? flockmates
...
to flock-mode  ;; herdanimal procedure
  find-flockmates
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < minimum-separation
...
to flock-shape-mode  ;; herdanimal procedure
  find-flockmates
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < minimum-separation
...
to find-flockmates  ;; herdanimal procedure
  set flockmates other herdanimals in-radius vision
end

to find-nearest-neighbor ;; herdanimal procedure
  set nearest-neighbor min-one-of flockmates [distance myself]
end

;;; SEPARATE

to separate  ;; herdanimal procedure
  turn-away ([heading] of nearest-neighbor) max-separate-turn
end

;;; ALIGN

to align  ;; herdanimal procedure
  turn-towards average-flockmate-heading max-align-turn
end

to-report average-flockmate-heading  ;; herdanimal procedure
...
to cohere  ;; herdanimal procedure
  turn-towards average-heading-towards-flockmates max-cohere-turn
end

to-report average-heading-towards-flockmates  ;; herdanimal procedure
  ;; "towards myself" gives us the heading from the other herdanimal
  ;; to me, but we want the heading from me to the other herdanimal,
  ;; so we add 180
...
to turn-towards [new-heading max-turn]  ;; herdanimal procedure
  turn-at-most (subtract-headings new-heading heading) max-turn
end

to turn-away [new-heading max-turn]  ;; herdanimal procedure
  turn-at-most (subtract-headings heading new-heading) max-turn
end

;; turn right by "turn" degrees (or left if "turn" is negative),
;; but never turn more than "max-turn" degrees
to turn-at-most [turn max-turn]  ;; herdanimal procedure
